...
import { MealTicket, TicketRequest, TicketApproval, TicketRedemption } from '../models/ticket.model';
...
export class TicketService {
  ...
  requestTicket(request: TicketRequest): Observable<MealTicket> {
    return this.http.post<MealTicket>(`${this.apiUrl}/employee/request-ticket`, request);
  }
  ...
  getTicketByNumber(ticketNumber: string): Observable<MealTicket> {
    return this.http.get<MealTicket>(`${this.apiUrl}/employee/ticket/${ticketNumber}`);
  }

  cancelTicket(ticketId: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/employee/cancel-ticket/${ticketId}`);
  }

  // Human Capital endpoints
  getPendingTickets(department?: string): Observable<MealTicket[]> {
    const params = department ? `?department=${department}` : '';
    return this.http.get<MealTicket[]>(`${this.apiUrl}/humancapital/pending-tickets${params}`);
  }

  approveTicket(approval: TicketApproval): Observable<any> {
    return this.http.post(`${this.apiUrl}/humancapital/approve-ticket`, approval);
  }
